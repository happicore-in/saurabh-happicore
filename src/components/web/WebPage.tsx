...
export const WebPage: React.FC<WebPageProps> = ({
  ...
  const flagshipProject =
    allWebProjects.find((p) => p.isFlagship) || allWebProjects[0];
  const selectedWebProjects = flagshipProject
    ? allWebProjects.filter((p) => p.id !== flagshipProject.id)
    : [];
  ...
  const showFlagship =
    flagshipProject && (activeFilter === 'all' || flagshipProject.filterType === activeFilter);
      ...
      <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isLoading ? (
          <div className="py-6 space-y-12">
            <FeaturedHeroSkeleton aspect="16/10" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <ProjectCardSkeleton aspectRatio="16/10" />
              <ProjectCardSkeleton aspectRatio="16/10" />
            </div>
          </div>
        ) : (
